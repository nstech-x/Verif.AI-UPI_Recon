...
def generate_matched_transactions_report(run_id: str) -> Optional[str]:
    ...
    for name in ["matched_transactions.csv", "matched.csv"]:
        ...
        if os.path.exists(candidate):
            return candidate
    # Fallback: compose from pairwise matched reports commonly generated by UPI flow.
    pairwise = [
    ...
    frames = []
    for fname in pairwise:
        path = os.path.join(reports_dir, fname)
        if os.path.exists(path):
            try:
                df = pd.read_csv(path)
                df["source_report"] = fname
                frames.append(df)
            except Exception:
                continue
    if frames:
        merged = pd.concat(frames, ignore_index=True)
        return _write_csv(run_id, "matched_transactions.csv", merged)
    return None
...
def generate_annexure_iv_split(run_id: str) -> Dict[str, Optional[str]]:
    ...
    for idx, exc in enumerate(exceptions, start=1):
        ...
        # Ensure unique and regex-safe Bankadjref per row to satisfy Annexure-IV strict validation.
        bankref_flag = str(flag).replace(" ", "")
        record = {
            "Bankadjref": f"BR_{bankref_flag}_{rrn}_{idx:06d}",
            "Flag": flag,
...
def generate_switch_status_update(run_id: str) -> Optional[str]:
    reports_dir = _reports_dir(run_id)
    preferred = os.path.join(reports_dir, "Switch_Update_File.csv")
    ...
    for exc in exceptions:
        ...
        rows.append({
            ...
            "RRN": rrn,
            "Old_Status": "",
            "New_Status": "PENDING_UPDATE",
            "Reason": str(exc.get("exception_type", ""))[:100],
            "Date": str(exc.get("date", ""))[:10],
            "Source_Systems": str(exc.get("source", "")),
        })

    if not rows:
        df = pd.DataFrame(columns=[
            "run_id", "generated_at", "RRN", "Old_Status", "New_Status", "Reason", "Date", "Source_Systems"
        ])
    else:
        df = pd.DataFrame(rows)

    return _write_csv(run_id, "Switch_Update_File.csv", df)
...
def find_gl_statement(run_id: str) -> Optional[str]:
    ...
    if os.path.exists(candidate):
        return candidate
    # UPI runs often generate Annexure-III and GL-vs files instead of gl_statement.csv.
    upi_candidates = [
        os.path.join(OUTPUT_DIR, run_id, "reports", "ANNEXURE_III.csv"),
        os.path.join(OUTPUT_DIR, run_id, "reports", "ANNEXURE_III.xlsx"),
        os.path.join(OUTPUT_DIR, run_id, "reports", "GL_vs_Switch_Inward.csv"),
        os.path.join(OUTPUT_DIR, run_id, "reports", "GL_vs_Switch_Outward.csv"),
        os.path.join(OUTPUT_DIR, run_id, "reports", "GL_vs_NPCI_Inward.csv"),
        os.path.join(OUTPUT_DIR, run_id, "reports", "GL_vs_NPCI_Outward.csv"),
    ]
    for path in upi_candidates:
        if os.path.exists(path):
            return path
